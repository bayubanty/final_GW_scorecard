...
// Enhanced data handling and mobile features
// ===============================
        ...
        detailCell.innerHTML = `
            <div class="detail-info">
                <p class="inline-stars"><strong>Outcome:</strong> 
                    ${renderStars(convertGradeToStars(hospital.TIER_2_GRADE_Outcome || hospital.Outcome_Grade || 'N/A').value)}
                </p>
                <p class="inline-stars"><strong>Value:</strong> 
                    ${renderStars(convertGradeToStars(hospital.TIER_2_GRADE_Value || hospital.Value_Grade || 'N/A').value)}
                </p>
                <p class="inline-stars"><strong>Civic:</strong> 
                    ${renderStars(convertGradeToStars(hospital.TIER_2_GRADE_Civic || hospital.Civic_Grade || 'N/A').value)}
                </p>
                <p class="inline-stars"><strong>Safety:</strong> 
                    ${renderStars(convertGradeToStars(hospital.TIER_3_GRADE_Pat_Saf || hospital.Safety_Grade || 'N/A').value)}
                </p>
                <p class="inline-stars"><strong>Experience:</strong> 
                    ${renderStars(convertGradeToStars(hospital.TIER_3_GRADE_Pat_Exp || hospital.Experience_Grade || 'N/A').value)}
                </p>
...
function updateMapMarkers(data) {
    console.log('Updating main map markers with', data.length, 'hospitals');
    ...
    if (data.length === 0) {
        console.log('No data to display on main map');
        return;
    }
    ...
    console.log('Added', mapMarkers.length, 'markers to main map');

    // Adjust map to fit all visible markers
    if (mapMarkers.length > 0) {
        const group = L.featureGroup(mapMarkers);
        map.fitBounds(group.getBounds().pad(0.2));
        console.log('Main map bounds adjusted to fit markers');
    }
...
function sortAndRender(data) {
    ...
    if (sortValue === 'grade') {
        sorted.sort((a, b) => {
            ...
            const gradeA = a.TIER_1_GRADE_Lown_Composite || a.Overall_Grade || 'N/A';
            const gradeB = b.TIER_1_GRADE_Lown_Composite || b.Overall_Grade || 'N/A';
